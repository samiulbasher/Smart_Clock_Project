...
void TFDrawChar (PxMATRIX* d, char value, char xo, char yo, int col)
{
  ...
  if (cfi > static_cast<int>(sizeof (tinyFont) / sizeof (TFFace)))
  {
...
#define DISPLAY_WIDTH  64   // Adjust as per your display width
void DrawText (PxMATRIX* d, String text, char xo, char yo, int col, bool center) {
  unsigned char lbuf[TFLINE_LEN + 1] = {0};
  text.getBytes(lbuf, TFLINE_LEN);

  int textLen = strlen((char*)lbuf);
  int textWidth = textLen * TF_COLS;

  // Centering logic if enabled
  if (center) {
    xo = (DISPLAY_WIDTH - textWidth) / 2;
  }

  for (unsigned char *lptr = lbuf; *lptr; lptr++, xo += TF_COLS) {
    TFDrawChar(d, *lptr, xo, yo, col);
  }
}

// print start from right to left
void DrawText_RTL(PxMATRIX* d, String text, char xo, char yo, int col) {
  ...
  xo = xo - textWidth;  // Shift starting point left by text width
